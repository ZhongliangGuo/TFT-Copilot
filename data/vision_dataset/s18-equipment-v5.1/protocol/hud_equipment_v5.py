...
ROOT = HERE.parent
PROFILE = dict(id='s18-equipment-v5', pitch=45.552433094138294, fov=32.339498296693534,
               horizontalTolerance=20, verticalRange=[40,240], equipmentLiftHypotheses=[0,28],
               equipmentOffsetSize=[-5,11,74,34], starOffsetSize=[-38,-11,40,26])
...
def locate(image, slots, occupied, mode='ally'):
    ...
    for result in assigned.values():
        ...
        if not (62 <= b['barBBox'][2] <= 70 and 2 <= b['barBBox'][3] <= 6):
            ...
            continue
        regions = dict(equipment=equipment_box(b))
        result['regions'] = {}
    ...
    return dict(schemaVersion=6, resolution=[1920,1080], profile=PROFILE, occupiedInputSource='first-stage predictions',
    ...
    for cell,result in manifest['slots'].items():
        ...
        for task,region in result.get('regions',{}).items():
            ...
            if not reasons:
                ...
                else:supervision['objects']=[dict(itemId=i['itemId'],bbox=[i['bbox'][0]-box[0],i['bbox'][1]-box[1],i['bbox'][2],i['bbox'][3]],iconBBox=[i['iconBBox'][0]-box[0],i['iconBBox'][1]-box[1],*i['iconBBox'][2:]],counterBBox=([i['counterBBox'][0]-box[0],i['counterBBox'][1]-box[1],*i['counterBBox'][2:]] if i.get('counterBBox') else None),counterValue=i.get('counterValue'),counterSource=i.get('counterSource')) for i in obj.get('equipment',[])]
